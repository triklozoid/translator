...
fn test_config_default() {
    ...
    assert_eq!(
        ...
        Language::from_iso_code_639_1(&IsoCode639_1::from_str("EN").unwrap())
    );
    assert_eq!(
        config.secondary_language,
        Language::from_iso_code_639_1(&IsoCode639_1::from_str("FR").unwrap())
    );
...
fn test_config_all_target_languages() {
    ...
    for language in expected_languages {
        assert!(
            config.all_target_languages.contains(&language),
            "Language {:?} not found in all_target_languages",
            language
        );
    }
}
...
fn test_config_save_permissions_error() {
    ...
    let mut dir_permissions = fs::metadata(&config_dir)
    ...
    assert!(
        result.is_err(),
        "Save should fail with permission error, but got: {:?}",
        result
    );
