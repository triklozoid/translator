...
fn test_language_detection() {
    ...
    let test_cases = vec![
        ...
        (
            "Bonjour le monde, ceci est un test du système de détection de langue.",
            Language::French,
        ),
        (
            "Hallo Welt, dies ist ein Test des Spracherkennungssystems.",
            Language::German,
        ),
        (
            "Hola mundo, esta es una prueba del sistema de detección de idiomas.",
            Language::Spanish,
        ),
        (
            "Ciao mondo, questo è un test del sistema di rilevamento della lingua.",
            Language::Italian,
        ),
    ];
    ...
    for (text, expected_lang) in test_cases {
        ...
        assert_eq!(
            detected,
            Some(expected_lang),
            "Failed to detect correct language for: {}",
            text
        );
    }
}
...
fn test_language_detection_with_short_text() {
    ...
    assert!(
        detected.is_some(),
        "Should return some language even for short text"
    );
}
