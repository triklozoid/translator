use gtk::prelude::*;
use gtk::{glib, gdk, Application, ApplicationWindow, Label, Button, ToggleButton, Box as GtkBox, Orientation, Align}; // Добавлен ToggleButton
use std::env;
use std::fs; // Для работы с файлами
use std::path::PathBuf; // Для работы с путями
use std::rc::Rc;
use std::cell::RefCell;
use dotenvy::dotenv;

// Imports for OpenAI API client
use async_openai::{
...
// Imports for Lingua
use lingua::{LanguageDetectorBuilder, Language}; // Removed unused IsoCode639_1
...
impl TargetLanguage {
    fn as_str(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Russian => "Russian", // Добавлено
        }
    }
    fn code(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Russian => "RU", // Добавлено
        }
    }
    ...
    fn from_code(code: &str) -> Option<Self> {
        match code {
            ...
            "RU" => Some(TargetLanguage::Russian), // Добавлено
            _ => None,
        }
    }
    // Helper to convert from lingua::Language
    fn from_lingua(lang: Language) -> Option<Self> {
        match lang {
    ...
    fn to_lingua(&self) -> Option<Language> {
        match self {
...
fn load_language_setting() -> Option<TargetLanguage> {
    let path = get_settings_path();
    match fs::read_to_string(path) {
        Ok(code) => TargetLanguage::from_code(code.trim()),
        Err(e) => {
            println!("Could not load language setting: {}", e); // Log error if needed
            None
        }
    }
}
...
fn save_language_setting(lang: TargetLanguage) {
    let path = get_settings_path();
...
// Removed the old update_active_button function that caused the error.
...
fn build_ui(app: &Application) {
        ...
        match clipboard.read_text_future().await {
            Ok(Some(gstring_text)) => { // text is glib::GString here
                ...
                *original_text_rc_clone_init.borrow_mut() = Some(text.clone()); // Store original text as String
                ...
                if detected_source_lang != Some(TargetLanguage::Russian) {
                    // Source is not Russian -> Target becomes Russian
                    final_target_lang = TargetLanguage::Russian;
                    println!("Source not Russian, switching target to RU");
                } else if current_target_lang == TargetLanguage::Russian {
                    // Source is Russian AND Target is Russian -> Target becomes English
                    final_target_lang = TargetLanguage::English;
                    println!("Source Russian, Target Russian, switching target to EN");
                } else {
                    // Source is Russian, Target is not Russian -> Keep target
    ...
    // REMOVED: This is now handled within the glib::spawn_future_local block
    ...
    | {
        ...
        // Clone all buttons for update_active_button inside handler
        // No need to clone these here, they are passed in `other_buttons` or are `button_rc`
            ...
            if toggled_button.is_active() {
                ...
                // We want to prevent this, ensuring one button is always selected.
                 if button_lang == *lang_rc.borrow() {
                     ...
                     // Corrected clone! macro usage (only need button_rc here)
...
use clone; // Make the macro available
